...
const { Event, Ticket } = require('../resources');
...
class Tickets {
  ...
  getByEmail (data) {
    logger.debug('ticket/getByEmail.entry:', data);
    ...
    return request(_.merge(this.requestOptions, options))
      .then((res) => {
        logger.debug('ticket/getByEmail.success:', res);
        const { data } = res;
        const tickets = data.map((item) => {
          return Ticket.from(item);
        });
        const foundTicket = _.find(tickets, (ticket) => {
      ...
      .catch((err) => {
        logger.error('ticket/getByEmail.error:', err);
        throw new Error('ticket/getByEmail.error');
      });
  }
  ...
  get (data) {
    ...
    if (!ticketId) {
      throw new Error('missing.option.TICKETID');
    }

    const endpoint = `/${event.slug}/${this.resource}/${ticketId}`;
    const options = {
      method: 'get',
      url: this.uri + endpoint
    };
